  ...
  async function handleGoogleSignIn() {
    ...
    try {
      const { data, error } = await supabase.auth.signInWithOAuth({
        provider: 'google',
        options: { redirectTo: 'village://auth-callback', skipBrowserRedirect: true },
      });
      if (error || !data.url) {
      ...
      if (result.type === 'success') {
        ...
        if (code) {
          ...
          if (at) {
            const { error: se } = await supabase.auth.setSession({ access_token: at, refresh_token: hash.get('refresh_token') ?? '' });
            if (se) Alert.alert('Sign-in failed', se.message);
          }
        }
      }
    } catch {
